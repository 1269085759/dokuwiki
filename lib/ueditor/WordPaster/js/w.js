...
function WordPasterManager()
{
	...
	this.Config = {
        ...
        , "FileFieldName"		: "upfile"//自定义文件名称名称
        //图片地址匹配规则配置教程：http://www.ncmem.com/doc/view.aspx?id=07e3f323d22d4571ad213441ab8530d1
        , "ImageMatch"		    : '\"url\":\"([^"]+)\"'//服务器返回数据匹配模式，正则表达式，提取括号中的地址
        //自定义图片地址配置教程：http://www.ncmem.com/doc/view.aspx?id=704cd302ebd346b486adf39cf4553936
        , "ImageUrl"		    : "http://localhost:811{url}"//自定义图片地址，格式"{url}"，{url}为固定变量，在此变量前后拼接图片路径，此变量的值为posturl返回的图片地址
        ...
        , "PostUrl"			    : "http://localhost:811/dokuwiki/lib/ueditor/php/controller.php?action=uploadimage"
        , "Fields"              : {}
        //x86
