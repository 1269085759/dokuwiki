...
function WordPasterManager()
{
    ...
    this.getJsDir = function () {
        ...
        for (var i = 0; i < js.length; i++) {
            if (js[i].src.indexOf("js/w.js") > -1) {
                jsPath = js[i].src.substring(0, js[i].src.indexOf("js/w.js"));
            }
        }
        return jsPath;
    };
    //http://localhost/WordPaster/
    var pathRoot = this.getJsDir();
    //http://localhost/WordPaster/css/
    var pathRes = pathRoot + "css/";
    ...
    UE.registerUI('wordpaster', function (editor, uiName) {
        ...
        var iconUrl = pathRes + 'paster.png';
    ...
    UE.registerUI('pdf', function (editor, uiName) {
        editor.registerCommand(uiName, {
            execCommand: function () {
                editor.focus();
                _this.SetEditor(editor);
    ...
    this.ui = { setup: null ,single:null,dialog:{files:false/**word图片上传窗口 */,paste:false/**粘贴窗口 */}};
    this.data={
	...
	this.Config = {
        ...
        , "IcoError"            : pathRes + "error.png"
        , "IcoUploader"         : pathRes + "upload.gif"
        //上传接口配置教程：http://www.ncmem.com/doc/view.aspx?id=d88b60a2b0204af1ba62fa66288203ed
        , "PostUrl"			    : pathRoot + "upload.php"
        , "Fields"              : {}
        //x86
